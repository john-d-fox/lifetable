...
The description is this section and the `lifetable()` function are based on @Barclay:1958. The arguments of the `lifetable()` function are
```{r lifetable-args}
args(lifetable)
```
where: 
...
* $l_x$ (the lower-case letter "el", `l[x]` in the output) is the number of individuals surviving to their $x$th birthday. 
...
* $T_x$ (`T[x]`) is the number of person-years lived after the $x$th birthday. $T_x$ simply cumulates $L_x$ from year $x$ on. A small censoring problem occurs at the end of the table if some individuals are still alive after the last year. One approach is to assume that those still alive live on average one more year. In the example table, 43 people are alive at the end of their 109th year.
...
The following shows the percentage of the synthetic cohort surviving at each age based on the single-year life table for females computed above and on a similar life table for males:
